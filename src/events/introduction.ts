...
export class Introduction extends Event {
  ...
  async intro(state: State) {
    ...
    await Write.standard(`To the North lie ${placeStyle(places.theMountains)}.`);
    await Write.standard(`To the East the ${placeStyle(places.forestOfNithe)}.`);
    await Write.standard(`To the South the ${placeStyle(places.diseasedLands)}.`);
    await Write.standard(`To the West is ${placeStyle(places.athosCityOfSails)}.`);
  }
